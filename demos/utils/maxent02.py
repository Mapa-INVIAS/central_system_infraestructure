...
# class GeoProcesamiento:

#     def preparar_carpetas(self, *subcarpetas):
#         """
#         Crea las carpetas necesarias dentro de MEDIA_ROOT si no existen.
#         """
#         for carpeta in subcarpetas:
#             path = os.path.join(settings.MEDIA_ROOT, carpeta)
#             os.makedirs(path, exist_ok=True)

#     def recortar_rasteres(self, raster_folder, crop_folder, line_shp_name, buffer_dist, simplify_factor):
#         """
#         Recorta rásteres usando un buffer simplificado generado desde un shapefile.
#         """
#         ruta_shp = os.path.join(settings.MEDIA_ROOT, line_shp_name)
#         if not os.path.exists(ruta_shp):
#             return

#         lineas = gpd.read_file(ruta_shp).to_crs(epsg=3857)
#         buffer_geom = lineas.buffer(buffer_dist).union_all().simplify(simplify_factor, preserve_topology=True)

#         ruta_buffer = os.path.join(settings.MEDIA_ROOT, "buffer_vias.shp")
#         gpd.GeoSeries([buffer_geom], crs='EPSG:3857').to_file(ruta_buffer)

#         carpeta_rasteres = os.path.join(settings.MEDIA_ROOT, raster_folder)
#         carpeta_crop = os.path.join(settings.MEDIA_ROOT, crop_folder)
#         os.makedirs(carpeta_crop, exist_ok=True)

#         raster_files = [f for f in os.listdir(carpeta_rasteres) if f.lower().endswith('.tif')]
#         if not raster_files:
#             return

#         for tif in tqdm(raster_files, desc="Recortando rásteres", leave=False, unit='raster'):
#             ruta_tif = os.path.join(carpeta_rasteres, tif)
#             try:
#                 with rasterio_open(ruta_tif) as src:
#                     buffer_geom_crs = gpd.GeoSeries([buffer_geom], crs='EPSG:3857').to_crs(src.crs)
...
#     def generar_puntos_aleatorios(self, crop_folder, output_sample_name, n_points):
#         """
#         Genera puntos aleatorios dentro del ráster recortado y los guarda como CSV.
#         """
#         carpeta_crop = os.path.join(settings.MEDIA_ROOT, crop_folder)
...
#                 sampled_indices = random.sample(indices, min(n_points, len(indices)))
#                 coords = [src.xy(row, col) for row, col in sampled_indices]

#                 df = pd.DataFrame(coords, columns=['x', 'y'])
#                 ruta_csv = os.path.join(settings.MEDIA_ROOT, output_sample_name)
