...
from .model.moverTifs import mover_tifs
...
def centralModelProcess():
    ...
    resultados = mover_tifs()
    resultados.run()

    # # Mover todos los archivos al directorio rasterIN
    # media_carga = Path(media_root, 'modula_carga', 'MOSAICS', 'EXPORTS')
    # media_servicios =  Path(media_root, 'modula_servicios', 'AutoINVIAS')
    # output_export = Path(media_root, 'modula_proceso', 'EXPORTS_TIF')
    ...
    #                 shutil.copy(full_path, output_export / file)

    # print("Extracción completada con estructura preservada.")
