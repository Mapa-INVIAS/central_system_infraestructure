...
# =========================
# Utils: encontrar carpeta más reciente
# =========================
...
def _to_2d_float_with_nan(arr) -> Optional[np.ndarray]:
    """
    Devuelve (H,W) float32; nodata/masked -> NaN. Si no es 2D tras squeeze, None.
    """
    if hasattr(arr, "mask"):
...
def compute_ref_grid(transforms, crs_list, res_list, arrays, prefer_crs=None):
    """
    Devuelve (out_crs, out_transform, out_width, out_height) como grilla común.
    Usa unión de bounds reproyectados y resolución mínima.
    """
    ...
    for a, T, crs in zip(arrays, transforms, crs_list):
        h, w = a.shape
        b = rasterio.transform.array_bounds(h, w, T)  # (miny? no, devuelve (miny?) -> (miny?) ) rasterio da (ymin,xmin?) -> ojo:
        # rasterio.transform.array_bounds retorna: (ymin? no) => (bottom, left, top, right)? NO.
        # En realidad: (south, west, north, east)??? -> mejor usar doc: retorna (ymin,xmin,ymax,xmax)??.
        # Para evitar confusión: asumimos orden (bottom, left, top, right)?? NO.
        # Lo correcto: array_bounds(height,width,transform) -> (ymin, xmin, ymax, xmax)???.
        # En práctica rasterio: (ymin,xmin,ymax,xmax) no; el orden es (west, south, east, north).
        # Entonces:
...
def mosaic_reproject_first(arrays, transforms, crs_list, dst_grid, resampling=Resampling.nearest):
    """
    Reproyecta cada array a la grilla común y compone por 'first valid' (primer pixel no-NaN).
    """
...
def save_tif_singleband(path: Path, arr_masked: np.ma.MaskedArray, T, crs, dtype_out: str):
    """
    Escribe GTiff 1 banda con nodata=0 y alpha mask (para ArcGIS/QGIS).
    """
...
def mosaic_product_fixedgrid(
    ...
    """
    Mosaica un producto multibanda (tiles con varias bandas) a:
      - tifs individuales por banda
      - multibanda final (a partir de los individuales)
    usando una grilla fija calculada con ref_band_name.
    """
...
def full_mosaic_nacional(exports_dir: Path, run_s2=True, run_hansen=True, run_srtm=True):
    ...
    s2_latest = find_latest_s2_folder(exports_dir) if run_s2 else None
    hansen_latest = find_latest_hansen_folder(exports_dir) if run_hansen else None
    srtm_latest = find_latest_srtm_folder(exports_dir) if run_srtm else None

    if run_s2 and s2_latest:
        mosaic_s2(s2_latest)
    if run_hansen and hansen_latest:
        mosaic_hansen(hansen_latest)
    if run_srtm and srtm_latest:
        mosaic_srtm(srtm_latest)

    return {
        "s2_latest": str(s2_latest) if s2_latest else None,
        "hansen_latest": str(hansen_latest) if hansen_latest else None,
        "srtm_latest": str(srtm_latest) if srtm_latest else None,
        "output": str(OUT_ROOT),
        "status": "OK"
    }
